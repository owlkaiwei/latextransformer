...
def start_matlab(option="-nodesktop"):
    ...
    with _engine_lock:
        eng = MatlabEngine(option=option)
...
def connect_matlab(name=None):
    ...
    with _engine_lock:
        if name is None:
            ...
            engines = find_matlab()
            if len(engines) == 0:
                eng = MatlabEngine(option="-r matlab.engine.shareEngine")
            else:
                #if there are shareable MATLAB sessions available
                eng = MatlabEngine(name=engines[0], attach=True)
        else:
            eng = MatlabEngine(name=name, attach=True)
        _engines.append(weakref.ref(eng))
